...
AMM的最大难点是如何实现价格发现机制。订单簿模式将这一功能交给市场，买卖双方各自出价，匹配上即能成交。但AMM买卖双方并不在同一时间进行交易，如何定价便成为了难题。
...
举个例子，假设有一个流动性池，包含 100 个 ETH 和 10,000 个 USDC，那么在初始状态下，`x` 为 100，`y` 为 10,000，因此 `k` 的值为 `100 * 10,000 = 1,000,000`。当有人希望用 USDC 换取 ETH 时，他们会向池中添加 USDC，同时从池中取出 ETH。为了保持乘积 `k` 不变，池中的 ETH 数量会减少，USDC 数量会增加，这样价格也随之变化。
...
在 Uniswap 中，手续费的被设计为一个固定的费率，例如每笔交易的手续费为 0.3%（在Uniswap V3 可以设置多档费率）。当用户进行代币交换时，这 0.3% 的手续费会添加到流动性池中，并按流动性提供者的份额比例分配给他们。
